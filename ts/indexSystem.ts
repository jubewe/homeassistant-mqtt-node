...
async function sendSystemState() {
  ...
  pm2.list((err, processes) => {
    if (err)
      return log(
        3,
        sn,
        Error("Error getting PM2 process list:", { cause: err })
      );
  ...
  r.info.memoryUse = parseInt(
    ((siData.mem.active / siData.mem.total) * 100).toFixed(2)
  );
  ...
  const siDataNetwork = await si.networkStats(
    await si.networkInterfaceDefault()
  );
  ...
  return new Promise<number>((resolve, reject) => {
    exec(
      (HAS_UNATTENDED_UPDATE
        ...
        `${HAS_SUDO ? "sudo " : ""}apt-get --just-print upgrade`,
      (error, stdout, stderr) => {
        if (error || stderr) {
          reject(error ?? stderr);
          return;
...
// ================= CLEAN EXIT =================
process.on("SIGINT", () => {
  log(1, sn, "Exiting, setting MQTT status to OFF");
